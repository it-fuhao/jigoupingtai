    ...
    $('#js_submit').on('click',function(){
        ...
        if(len > 0 || $.trim(text) !== ''){
            ...
            var params = {
                'list': arr,
                'text': $.trim(text),
				'pre_id':$('.main_box').attr('data-id')
            };

            //requestData(API.APPROVE.NOCONSENT, 'post', function(data){

			requestData('/ManagePrepare/disagreePrepareSubmit', 'post', function(data){
                if (data.status == 1) {
                    window.location = data.msg;
                } else {
